...
function primaryCallback() {
  ...
  const modifiers = {
    ...
    formTitle: "Schedule a Call-Back",
...
//    modal.appendChild(modalSubHeading);
    ...
    requestType.setAttribute("style", `font-size: 12px; border-width: 4px; text-align: center; margin-bottom: 0px; margin-top: 0px; border-top:0; border-left: 0; border-right: 0; grid-column: 1 / 3;`);
    ...
    confirmButton.addEventListener("click", async function validate(e) {
      ...
      try {
        ...
        let response = await fetch(IMI_CALLBACK_WEBHOOK, {
          method: "POST",
          headers: {
        ...
        console.log("Callbackdata: ", data.response[0].description);
        let obj = data.response[0].description;
        console.log("CallbackStatus: ", obj);
